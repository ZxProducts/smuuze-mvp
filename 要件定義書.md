...
- **データベース**
  ...
  - ユーザー、チーム、プロジェクト、タスク、タイムエントリー、レポート、オファー等を管理
...
### 3.2. チーム・メンバー管理
- **チーム作成・編集**
  - チーム名、概要、メンバー一覧、各メンバーの役割、単価、特記事項の管理
- **オファー機能**
  - 管理者がメンバー登録時にオファーを作成  
    - 入力項目：時間単価、日次稼働時間目安、週稼働日数目安、社内会議費の請求対象、特記事項
  - 作成後、対象メンバーへメールで承諾リンクを送信
  - メンバーが承諾すると、システム上で自動的にチームメンバーとして追加
...
3. **オファー承諾**
   - メール内リンクからオファー承諾ページに遷移し、承諾後に自動的にチームメンバーとして登録
...
| id          | UUID (PK)               | チームID                                |
| name        | TEXT                    | チーム名                                |
| description | TEXT                    | チームの概要・説明                       |
| created_by  | UUID                    | チーム作成者（`profiles.id` への参照）      |
| created_at  | TIMESTAMP WITH TZ       | 作成日時                                |
| updated_at  | TIMESTAMP WITH TZ       | 更新日時                                |

### 6.3. `team_members`
チームに所属するメンバー情報および契約情報

| カラム名          | データ型          | 説明                                               |
| ----------------- | ----------------- | -------------------------------------------------- |
| id                | UUID (PK)         | レコードID                                         |
| team_id           | UUID                | チームID（`teams.id` への参照）                      |
...
| notes             | TEXT                | 特記事項                                           |
| joined_at         | TIMESTAMP WITH TZ   | チーム参加日時                                    |
...
| ----------- | ----------------------- | ---------------------------------------- |
| id          | UUID (PK)               | プロジェクトID                            |
| team_id     | UUID                | 所属チームID（`teams.id` への参照）          |
| name        | TEXT                    | プロジェクト名                            |
...
| team_id     | UUID                | チームID（`teams.id` への参照、クエリ効率向上のため） |
| title       | TEXT                    | タスクタイトル                              |
